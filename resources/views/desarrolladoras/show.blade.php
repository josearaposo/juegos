...
{{--             @if ($desarrolladora->existeImagen())
     ...
     <a href="{{route('hacer_comentario', ['comentable' => $desarrolladora, 'tipo' =>  'desarrolladora', 'desarrolladora' => $desarrolladora])}}">
        Hacer comentarios
    </a>
    </button>

            <a href="{{ route('desarrolladoras.index') }}" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
                Volver
            </a>
        </div>
        {{ $desarrolladora->mostrar_comentarios()}}
    </div>
</x-app-layout>
